...
export default function Header() {
  ...
  useEffect(() => {
    async function fetchCounts() {
      ...
      const { count: cartCnt, error: cartError } = await supabase
        .from("cart_items")
        .select("*", { count: "exact", head: true })
        .eq("user_id", user.id);

      if (!cartError && typeof cartCnt === "number") {
        setCartCount(cartCnt);
      }

      const { count: orderCnt, error: orderError } = await supabase
        .from("orders")
        .select("*", { count: "exact", head: true })
        .eq("user_id", user.id);

      if (!orderError && typeof orderCnt === "number") {
      ...
      <header className="fixed top-0 left-0 w-full z-50 px-6 py-4 flex justify-end">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`relative w-14 h-14 rounded-full flex flex-col justify-center items-center transition-all hover:scale-105 shadow-lg cursor-pointer ${
          ...
          <nav className="flex flex-col space-y-2 flex-1">
            {navItems.map((item, i) => (
              <Link
                ...
                href={item.path}
                onClick={closeMobileMenu}
                className={`relative text-[2rem] md:text-[2.5rem] hover:opacity-80 transition-all duration-500 ease-out pb-3 transform ${
                  isMobileMenuOpen
                    ? "translate-x-0 opacity-100"
              ...
              >
                ...
                {item.name === "Orders" && orderCount > 0 && (
                  <span className="absolute top-2 md:left-30  bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">
                    {orderCount}
                  </span>
                )}
              </Link>
